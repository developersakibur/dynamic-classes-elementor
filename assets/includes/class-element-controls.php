 ...
 * of Elementor Containers, Sections, Columns, and Widgets,
 * letting users pick a saved spacing class directly in the editor.
 *
...
class DCE_Element_Controls {
     ...
     * The generator's internal cache means kit_manager is only
     * queried once per type regardless of how many elements render.
     *
    ...
    public function add_controls( $element, $args ): void {
        ...
        if ( ! $is_structural ) {
            ...
            if (
                count( $gap_opts ) <= 1 &&
                count( $padding_opts ) <= 1 &&
                count( $margin_opts ) <= 1 &&
                count( $min_height_opts ) <= 1 &&
                count( $max_width_opts ) <= 1
            ) {
                return;
            }
        }
